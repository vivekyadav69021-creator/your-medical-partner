...
const PrescriptionOutputSchema = z.object({
  identifiedMedicines: z.array(z.string()).describe('A complete list of all medicine names identified from the prescription image. This should be the first and most comprehensive list.'),
  availableMedicines: z.array(z.string()).describe('A list of medicines from the identified list that are available in the store.'),
  unidentifiedMedicines: z.array(z.string()).describe('A list of medicines from the identified list that are NOT available in the store.'),
});
...
const prompt = ai.definePrompt({
  ...
  **Instructions:**
  ...
  2.  **List ALL Identified Medicines:** First, create a complete list of every single medicine name you can identify from the image and put it in the 'identifiedMedicines' field.
  ...
  4.  **Create Final Lists:**
      - Populate 'availableMedicines' with the names from your 'identifiedMedicines' list that are also present in our available stock list. Match them even if there are minor spelling differences (e.g., "Paracitamol" should match "Paracetamol 500mg").
      - Populate 'unidentifiedMedicines' with the names from your 'identifiedMedicines' list that are NOT in our available stock list.
